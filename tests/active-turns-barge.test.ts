...
test('stopFor(clearQueue:false) kills without marking stopped; stop() marks stopped', () => {
  ...
  assert.equal(activeTurns.consumeSteered(c1), true, 'barge is visible to the renderer once')
  assert.equal(activeTurns.consumeSteered(c1), false)
  ...
  assert.equal(activeTurns.consumeSteered(c2), false, 'explicit stop is not steering')
  activeTurns.done(c1)
  activeTurns.done(c2)
})
