...
test('stopFor(clearQueue:false) kills without marking stopped; stop() marks stopped', () => {
  ...
  assert.ok(activeTurns.consumeSteered(c1)! >= 0, 'barge exposes elapsed time once')
  assert.equal(activeTurns.consumeSteered(c1), null)
  ...
  assert.equal(activeTurns.consumeSteered(c2), null, 'explicit stop is not steering')
  activeTurns.done(c1)
  activeTurns.done(c2)
})
