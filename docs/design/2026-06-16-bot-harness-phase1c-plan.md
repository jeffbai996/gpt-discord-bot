...
## Deploy
After review + merge: host-a `git checkout -- package-lock.json` (churn) → `git pull --ff-only` → `npm install` → restart `gpt.service` → confirm `gpt online` + Gem replies normally (behavior-preservation is the whole point).
